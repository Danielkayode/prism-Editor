...
export class CodeApplication extends Disposable {
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		mainProcessElectronServer.registerChannel('prism-channel-metrics', metricsChannel);

		const voidUpdatesChannel = ProxyChannel.fromService(accessor.get(IVoidUpdateService), disposables);
		mainProcessElectronServer.registerChannel('prism-channel-update', voidUpdatesChannel);

		const sendLLMMessageChannel = new LLMMessageChannel(accessor.get(IMetricsService));
		mainProcessElectronServer.registerChannel('prism-channel-llmMessage', sendLLMMessageChannel);

		// Void added this
		const voidSCMChannel = ProxyChannel.fromService(accessor.get(IVoidSCMService), disposables);
		mainProcessElectronServer.registerChannel('prism-channel-scm', voidSCMChannel);

		// Void added this
		const mcpChannel = new MCPChannel();
		mainProcessElectronServer.registerChannel('prism-channel-mcp', mcpChannel);
