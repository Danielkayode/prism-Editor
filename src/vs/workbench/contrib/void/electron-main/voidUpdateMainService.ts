...
import { IPrismUpdateService } from '../common/voidUpdateService.js';
import { PrismCheckUpdateRespose } from '../common/voidUpdateServiceTypes.js';



export class PrismMainUpdateService extends Disposable implements IPrismUpdateService {
	...
	async check(explicit: boolean): Promise<PrismCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Downloaded) { // Fixed: was using 'state.type' which is not defined
			return { message: 'Restart Prism to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<PrismCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/prismeditor/binaries/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
		if (compare(res.tag_name, currentTag) > 0) {
			message = 'A new version of Prism is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
			action = 'reinstall'
		}
		else {
			message = 'Prism is up-to-date!'
			action = undefined
		}
				}
				else {
