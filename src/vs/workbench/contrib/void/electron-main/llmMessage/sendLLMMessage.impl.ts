...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({
			...
			defaultHeaders: {
				'HTTP-Referer': 'https://prismeditor.com', // Optional, for including your app on openrouter.ai rankings.
				'X-Title': 'Prism', // Optional. Shows in rankings on openrouter.ai.
			},
			...commonPayloadOpts,
		})
	}
		...
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://api.deepinfra.com/v1/openai', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}
		...
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://glama.ai/api/v1', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}
	else if (providerName === 'unbound') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://api.getunbound.ai/v1', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}
	else if (providerName === 'requesty') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://router.requesty.ai/v1', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}
	else if (providerName === 'baseten') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://bridge.baseten.co/v1', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}
	else if (providerName === 'chutes') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://chutes.ai/v1', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}
	else if (providerName === 'doubao') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: thisConfig.endpoint || 'https://ark.cn-beijing.volces.com/api/v3', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}
	else if (providerName === 'huggingface') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: thisConfig.endpoint || 'https://api-inference.huggingface.co/v1', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}
		...
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://api.io-intelligence.ai/v1', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}
		...
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://endpoints.ai.cloud.ovh.net/v1', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}
		...
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://api.z.ai/api/paas/v4', apiKey: thisConfig.apiKey, ...commonPayloadOpts })
	}

	else throw new Error(`Prism providerName was invalid: ${providerName}.`)
}
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'Prism: Response from model was empty.', fullError: null })
			}
			else {
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
	...
	if (!endpoint) throw new Error(`Ollama Endpoint was empty (please enter ${defaultProviderSettings.ollama.endpoint} in Prism if you want the default url).`)
