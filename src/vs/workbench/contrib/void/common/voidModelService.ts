...
export interface IPrismModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): PrismModelType;
	getModelFromFsPath(fsPath: string): PrismModelType;
	getModelSafe(uri: URI): Promise<PrismModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const IPrismModelService = createDecorator<IPrismModelService>('prismPrismModelService');

class PrismModelService extends Disposable implements IPrismModelService {
	_serviceBrand: undefined;
	static readonly ID = 'prismPrismModelService';
	...
	getModelSafe = async (uri: URI): Promise<PrismModelType> => {
...
registerSingleton(IPrismModelService, PrismModelService, InstantiationType.Eager);
