...
export type PrismSettingsState = {
...
// type RealPrismSettings = Exclude<keyof PrismSettingsState, '_modelOptions'>
// type EventProp<T extends RealPrismSettings = RealPrismSettings> = T extends 'globalSettings' ? [T, keyof PrismSettingsState[T]] : T | 'all'


export interface IPrismSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: PrismSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: PrismSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: PrismStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, PrismStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: PrismSettingsState): PrismSettingsState => {
...
const _validatedModelState = (state: Omit<PrismSettingsState, '_modelOptions'>): PrismSettingsState => {
...
export const IPrismSettingsService = createDecorator<IPrismSettingsService>('prismSettingsService');
class PrismSettingsService extends Disposable implements IPrismSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: PrismSettingsState;
	...
	dangerousSetState = async (newState: PrismSettingsState) => {
	...
	private async _readState(): Promise<PrismSettingsState> {
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: PrismSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: PrismStatefulModelInfo[] = [
...
registerSingleton(IPrismSettingsService, PrismSettingsService, InstantiationType.Eager);
