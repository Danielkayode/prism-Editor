...
export interface IMCPService {
	...
	installMCPServer(serverName: string, config: MCPConfigFileEntryJSON): Promise<void>;
...
class MCPService extends Disposable implements IMCPService {
	...
	constructor(
		...
		super();
		this.channel = this.mainProcessService.getChannel('prism-channel-mcp')
	...
	// toggle MCP server and update isOn in prism settings
	...
	public async installMCPServer(serverName: string, config: MCPConfigFileEntryJSON): Promise<void> {
		const mcpConfigUri = await this._getMCPConfigFilePath();
		const currentConfig = await this._parseMCPConfigFile() || { mcpServers: {} };
		...
		// The file watcher will trigger _refreshMCPServers automatically
	}
