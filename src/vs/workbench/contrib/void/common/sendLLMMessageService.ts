...
export class LLMMessageService extends Disposable implements ILLMMessageService {
	...
	constructor(
		...
		// const service = ProxyChannel.toService<LLMMessageChannel>(mainProcessService.getChannel('prism-channel-llmMessage')); // lets you call it like a service
		// see llmMessageChannel.ts
		this.channel = this.mainProcessService.getChannel('prism-channel-llmMessage')
	...
	sendLLMMessage(params: ServiceSendLLMMessageParams) {
		...
		// throw an error if no model/provider selected (this should usually never be reached, the UI should check this first, but might happen in cases like Apply where we haven't built much UI/checks yet, good practice to have check logic on backend)
		        if (modelSelection === null) {
		            const message = `Please add a provider in Prism's Settings.`
