...
export type PrismStaticModelInfo = { // not stateful
	// Prism uses the information below to know how to handle each model.
...
type PrismStaticProviderInfo = { // doesn't change (not stateful)
	...
	modelOptions: { [key: string]: PrismStaticModelInfo };
	modelOptionsFallback: (modelName: string, fallbackKnownValues?: Partial<PrismStaticModelInfo>) => (PrismStaticModelInfo & { modelName: string, recognizedModelName: string }) | null;
}
...
const defaultModelOptions = {
	...
	supportsFIM: false,
	reasoningCapabilities: false,
} as const satisfies PrismStaticModelInfo
...
} as const satisfies { [s: string]: Partial<PrismStaticModelInfo> }




// keep modelName, but use the fallback's defaults
const extensiveModelOptionsFallback: PrismStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {

	const lower = modelName.toLowerCase()

	const toFallback = <T extends { [s: string]: Omit<PrismStaticModelInfo, 'cost' | 'downloadable'> },>(obj: T, recognizedModelName: string & keyof T)
		: PrismStaticModelInfo & { modelName: string, recognizedModelName: string } => {
	...
	if (lower.includes('gemini') && (lower.includes('3') || lower.includes('3-0'))) return toFallback(geminiModelOptions, 'gemini-3-pro-preview')
	...
	if (lower.includes('claude-4')) return toFallback(anthropicModelOptions, 'claude-sonnet-4-5')
	...
	if (lower.includes('deepseek-v3') || lower.includes('deepseek-v3')) return toFallback(openSourceModelOptions_assumingOAICompat, 'deepseekCoderV3')
	...
	if (lower.includes('quaser')) return toFallback(openSourceModelOptions_assumingOAICompat, 'quasar')
	if (lower.includes('quasar')) return toFallback(openSourceModelOptions_assumingOAICompat, 'quasar')

	if (lower.includes('minimax')) return toFallback(openSourceModelOptions_assumingOAICompat, 'deepseekCoderV3')
	if (lower.includes('kimi') || lower.includes('moonshot')) return toFallback(openSourceModelOptions_assumingOAICompat, 'qwq')
	if (lower.includes('glm-4') || lower.includes('zai')) return toFallback(openSourceModelOptions_assumingOAICompat, 'qwen3')
	if (lower.includes('gpt-oss')) return toFallback(openSourceModelOptions_assumingOAICompat, 'gpt-oss')

	if (lower.includes('gpt-5') || lower.includes('gpt-5')) return toFallback(openAIModelOptions, 'gpt-5')
...
} as const satisfies { [s: string]: PrismStaticModelInfo }

const anthropicSettings: PrismStaticProviderInfo = {
...
const openAIModelOptions = { // https://platform.openai.com/docs/pricing
	'o3-pro': {
		...
		reasoningCapabilities: { supportsReasoning: true, canTurnOffReasoning: false, canIOReasoning: false, reasoningSlider: { type: 'effort_slider', values: ['low', 'medium', 'high', 'xhigh'], default: 'high' } },
	},
	'gpt-5.2-codex': {
...
const openAISettings: PrismStaticProviderInfo = {
...
const xAISettings: PrismStaticProviderInfo = {
...
const geminiModelOptions = { // https://ai.google.dev/gemini-api/docs/pricing
	...
	'gemini-2.5-pro': {
		contextWindow: 200_000,
		reservedOutputTokenSpace: 64_000,
		cost: { input: 2.5, output: 15.0, cache_read: 0.625, cache_write: 4.5 },
		downloadable: false,
		supportsFIM: false,
		supportsSystemMessage: 'separated',
		specialToolFormat: 'gemini-style',
		reasoningCapabilities: {
			...
			canTurnOffReasoning: true,
			canIOReasoning: false,
			reasoningSlider: { type: 'budget_slider', min: 1024, max: 32768, default: 1024 },
			reasoningReservedOutputTokenSpace: 8192,
		},
	},
	'gemini-2.5-flash': {
		contextWindow: 200_000,
		reservedOutputTokenSpace: 64_000,
		cost: { input: 0.3, output: 2.5, cache_read: 0.075, cache_write: 1.0 },
		downloadable: false,
		supportsFIM: false,
		supportsSystemMessage: 'separated',
		specialToolFormat: 'gemini-style',
...
const geminiSettings: PrismStaticProviderInfo = {
...
const deepseekSettings: PrismStaticProviderInfo = {
...
const mistralSettings: PrismStaticProviderInfo = {
...
const groqSettings: PrismStaticProviderInfo = {
...
} as const satisfies Record<string, PrismStaticModelInfo>
const googleVertexSettings: PrismStaticProviderInfo = {
...
const microsoftAzureSettings: PrismStaticProviderInfo = {
...
const awsBedrockSettings: PrismStaticProviderInfo = {
...
const vLLMSettings: PrismStaticProviderInfo = {
...
const lmStudioSettings: PrismStaticProviderInfo = {
...
const ollamaSettings: PrismStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
...
const openaiCompatible: PrismStaticProviderInfo = {
...
const liteLLMSettings: PrismStaticProviderInfo = { // https://docs.litellm.ai/docs/reasoning_content
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
	modelOptions: {},
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const openRouterSettings: PrismStaticProviderInfo = {
...
const modelSettingsOfProvider: { [providerName in ProviderName]: PrismStaticProviderInfo } = {
	...
	fireworks: {
		modelOptions: {},
		modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName),
		providerReasoningIOSettings: { input: { includeInPayload: openAICompatIncludeInPayloadReasoning } },
	},
	together: {
		modelOptions: {},
		modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName),
