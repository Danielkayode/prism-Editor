...
import { PrismFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export const IPrismChatThreadService = createDecorator<IPrismChatThreadService>('prismChatThreadService');
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const voidFileSnapshot = checkpointMessage.prismFileSnapshotOfURI ? checkpointMessage.prismFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { voidFileSnapshot, } }

		const userModifiedPrismFileSnapshot = fsPath in checkpointMessage.userModifications.prismFileSnapshotOfURI ? checkpointMessage.userModifications.prismFileSnapshotOfURI[fsPath] ?? null : null
		return { voidFileSnapshot: userModifiedPrismFileSnapshot ?? voidFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const prismFileSnapshotOfURI: { [fsPath: string]: PrismFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			...
			if (!res) continue
			const { voidFileSnapshot: oldPrismFileSnapshot } = res
			...
			const voidFileSnapshot = this._editCodeService.getPrismFileSnapshot(URI.file(fsPath))
			if (oldPrismFileSnapshot === voidFileSnapshot) continue
			prismFileSnapshotOfURI[fsPath] = voidFileSnapshot
		}
		...
		return { prismFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { prismFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			prismFileSnapshotOfURI: prismFileSnapshotOfURI ?? {},
			userModifications: { prismFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getPrismFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			prismFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { prismFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.prismFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { prismFileSnapshotOfURI: prismFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					this._editCodeService.restorePrismFileSnapshot(URI.file(fsPath), voidFileSnapshot)
					break
				}
			}
		}
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._prismModelService.getModelSafe(uri)
