...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('prismGenerateCommitMessageService');

const loadingContextKey = 'prismSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private prismSCM: IPrismSCMService
	...
	constructor(
		...
		this.prismSCM = ProxyChannel.toService<IPrismSCMService>(mainProcessService.getChannel('prism-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.prismSCM.gitStat(path),
					this.prismSCM.gitSampledDiffs(path),
					this.prismSCM.gitBranch(path),
					this.prismSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.prismSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.prismSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.prismSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'PrismSCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('prismFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'prism.generateCommitMessageAction',
			title: localize2('prismCommitMessagePrompt', 'Prism: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('prismCommitMessagePromptTooltip', 'Prism: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'prism.loadingGenerateCommitMessageAction',
			title: localize2('prismCommitMessagePromptCancel', 'Prism: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('prismCommitMessagePromptCancelTooltip', 'Prism: Cancel Commit Message Generation'),
