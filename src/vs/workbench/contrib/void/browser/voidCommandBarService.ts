...
import { mountPrismCommandBar } from './react/out/prism-editor-widgets-tsx/index.js'
...
import { IPrismModelService } from '../common/prismModelService.js';



export interface IPrismCommandBarService {
	...
	setDiffIdx(uri: URI, newIdx: number | null): prism;

	getNextDiffIdx(step: 1 | -1): number | null;
	getNextUriIdx(step: 1 | -1): number | null;
	goToDiffIdx(idx: number | null): prism;
	goToURIIdx(idx: number | null): Promise<prism>;

	acceptOrRejectAllFiles(opts: { behavior: 'reject' | 'accept' }): prism;
	anyFileIsStreaming(): boolean;

}


export const IPrismCommandBarService = createDecorator<IPrismCommandBarService>('PrismCommandBarService');
...
export class PrismCommandBarService extends Disposable implements IPrismCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'prism.PrismCommandBarService'
	...
	constructor(
		...
		@IPrismModelService private readonly _prismModelService: IPrismModelService,
	) {
		super();
	...
	async goToURIIdx(idx: number | null): Promise<prism> {
		...
		const { model } = await this._prismModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IPrismCommandBarService, PrismCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type PrismCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountPrismCommandBar(root, accessor, { uri, editor } satisfies PrismCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies PrismCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('prismAcceptDiffAction', 'Prism: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.PrismExtension,
			}
		});
	}

	async run(accessor: ServicesAccessor): Promise<prism> {
		const editCodeService = accessor.get(IEditCodeService);
		const commandBarService = accessor.get(IPrismCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('prismRejectDiffAction', 'Prism: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('prismGoToNextDiffAction', 'Prism: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('prismGoToPrevDiffAction', 'Prism: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('prismGoToNextUriAction', 'Prism: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('prismGoToPrevUriAction', 'Prism: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('prismAcceptFileAction', 'Prism: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('prismRejectFileAction', 'Prism: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('prismAcceptAllDiffsAction', 'Prism: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('prismRejectAllDiffsAction', 'Prism: Reject All Diffs in All Files'),
			keybinding: {
