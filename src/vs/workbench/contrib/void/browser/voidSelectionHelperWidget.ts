...
import { mountPrismSelectionHelper } from './react/out/void-editor-widgets-tsx/index.js';
...
export type PrismSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.prismSelectionHelper';
	// react
	...
	constructor(
		...
		@IPrismSettingsService private readonly _prismSettingsService: IPrismSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountPrismSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._prismSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies PrismSelectionHelperProps)
