						...
						<div
							...
							onClick={() => {
								...
								if (isThisSelectionProspective) { // add prospective selection to selections
									...
									prismOpenFileFn(selection.uri, accessor);
									...
									prismOpenFileFn(selection.uri, accessor, selection.range);
								}
...
const EditTool = ({ toolMessage, threadId, messageIdx, content }: Parameters<ResultWrapper<'edit_file' | 'rewrite_file'>>[0] & { content: string }) => {
	...
	const desc1OnClick = () => prismOpenFileFn(params.uri, accessor)
...
const builtinToolNameToComponent: { [T in BuiltinToolName]: { resultWrapper: ResultWrapper<T>, } } = {
	'read_file': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				const { result } = toolMessage
				componentParams.onClick = () => { prismOpenFileFn(params.uri, accessor, range) }
				...
				componentParams.children = !result.children || (result.children.length ?? 0) === 0 ? undefined
					: <ToolChildrenWrapper>
						{result.children.map((child, i) => (<ListableToolItem key={i}
							...
							onClick={() => {
								prismOpenFileFn(child.uri, accessor)
				...
				componentParams.children = result.uris.length === 0 ? undefined
					: <ToolChildrenWrapper>
						{result.uris.map((uri, i) => (<ListableToolItem key={i}
							...
							className='w-full overflow-auto'
							onClick={() => { prismOpenFileFn(uri, accessor) }}
						/>))}
						{result.hasNextPage &&
	...
	'read_lint_errors': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				const { result } = toolMessage
				componentParams.onClick = () => { prismOpenFileFn(params.uri, accessor) }
	...
	'create_file_or_folder': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				...
				const { result } = toolMessage
				if (params) { componentParams.onClick = () => { prismOpenFileFn(params.uri, accessor) } }
				componentParams.bottomChildren = <BottomChildren title='Error'>
					<CodeChildren>
						{result}
					</CodeChildren>
...
const EditToolSoFar = ({ toolCallSoFar, }: { toolCallSoFar: RawToolCallObj }) => {
	...
	const desc1OnClick = () => { uri && prismOpenFileFn(uri, accessor) }
...
export const SidebarChat = () => {
	...
	>
		<VoidInputBox2
			...
			placeholder={`@ to mention, ${keybindingString ? `${keybindingString} to add a selection. ` : ''}Ask Prism something...`}
	...
	const initiallySuggestedPromptsHTML = <div className='flex flex-col gap-2 w-full text-nowrap text-void-fg-3 select-none'>
		{[
			...
			'Create a .prismrules file for me'
		].map((text, index) => (
			<div
				key={index}
