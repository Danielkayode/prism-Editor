...
import { getBasename, getRelative, prismOpenFileFn } from '../sidebar-tsx/SidebarChat.js'
...
const CodespanWithLink = ({ text, rawText, chatMessageLocation }: { text: string, rawText: string, chatMessageLocation: ChatMessageLocation }) => {
	...
	const onClick = () => {
		if (!link) return;
		// Use the updated prismOpenFileFn to open the file and handle selection
		if (link.selection)
			prismOpenFileFn(link.uri, accessor, [link.selection.startLineNumber, link.selection.endLineNumber]);
		else
			prismOpenFileFn(link.uri, accessor);
	}
