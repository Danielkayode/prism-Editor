		...
		>
			...
			>
				...
				<div className="text-sm text-void-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with Prism, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by Prism.`
							: `Prism recognizes ${modelName} ("${recognizedModelName}").`}
				</div>
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-void-fg-3 text-sm list-decimal select-text'>
		...
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`Prism automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const MCPMarketplace = () => {
	...
	const fetchMarketplace = useCallback(async () => {
		...
		setError(null);
		try {
			// Using cline's marketplace API as a reference/source
			...
			// Ensure it's an array or has an items property
			const items = Array.isArray(data) ? data : (data.items || []);
			setMarketplaceMcps(items);
		} catch (err) {
	...
	const filteredMcps = useMemo(() => {
		if (!searchQuery) return marketplaceMcps;
		const lowerQuery = searchQuery.toLowerCase();
		return marketplaceMcps.filter(mcp =>
			(mcp.name || '').toLowerCase().includes(lowerQuery) ||
			(mcp.description || '').toLowerCase().includes(lowerQuery)
		);
	}, [marketplaceMcps, searchQuery]);

	const installMcp = async (mcp: any) => {
		try {
			notificationService.info(`Installing ${mcp.name}...`);

			// Get content from the marketplace item
			let contentToUse: string;
			if (Array.isArray(mcp.content)) {
				// Use the first installation method if it's an array
				contentToUse = mcp.content[0].content;
			} else {
				contentToUse = mcp.content;
			}

			if (!contentToUse) {
				throw new Error('No content found for this MCP server');
			}

			const config = JSON.parse(contentToUse);
			await mcpService.installMCPServer(mcp.id || mcp.name, config);

			notificationService.info(`Successfully installed ${mcp.name}!`);

		} catch (err) {
			notificationService.error(`Failed to install ${mcp.name}: ${err}`);
		}
	};
		...
		<div className="mt-4">
			...
			{isLoading && (
				<div className="flex justify-center p-8">
					<Loader2 className="animate-spin size-8 text-void-fg-3" />
				</div>
			)}
			...
			{!isLoading && !error && (
				<div className="grid grid-cols-1 gap-4">
					{filteredMcps.map((mcp) => (
						<div key={mcp.id || mcp.name} className="border border-void-border-2 bg-void-bg-1 p-4 rounded-sm flex flex-col gap-2">
							<div className="flex justify-between items-start">
								<div className="font-medium text-lg text-void-fg-1">{mcp.name}</div>
								<button
									onClick={() => installMcp(mcp)}
									className="bg-[#0e70c0] px-3 py-1 text-white rounded-sm hover:bg-[#1177cb] transition-colors text-sm"
								>
									Install
								</button>
							</div>
							<div className="text-sm text-void-fg-3 line-clamp-2">{mcp.description}</div>
							{mcp.author && <div className="text-xs text-void-fg-4 italic">by {mcp.author}</div>}
						</div>
					))}
					{filteredMcps.length === 0 && (
						<div className="text-center text-void-fg-3 py-8">No MCP servers found matching your search.</div>
					)}
				</div>
			)}
		</div>
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`Prism's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`Prism can access any model that you host locally. We automatically detect your local models by default.`}</h3>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`Prism can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>
								...
								<ErrorBoundary>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-void-fg-3 mt-1'>{`Settings that control the visibility of Prism suggestions in the code editor.`}</div>
							...
							{/* Marketplace section */}
							<div className={shouldShowTab('marketplace') ? `` : 'hidden'}>
								<ErrorBoundary>
									<h2 className='text-3xl mb-2'>Marketplace</h2>
									<h4 className={`text-void-fg-3 mb-4`}>
										Discover and install MCP servers to extend Prism's capabilities.
									</h4>
									<ErrorBoundary>
										<MCPMarketplace />
									</ErrorBoundary>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-void-fg-3 mb-4'>{`Transfer your editor settings into Prism.`}</h4>
								...
								<div>
									...
									<h4 className='text-void-fg-3 mb-4'>{`Transfer Prism's settings and chats in and out of Prism.`}</h4>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-void-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep Prism running smoothly. You may opt out below. Regardless of this setting, Prism never sees your code, messages, or API keys.</h4>
...
Alternatively, place a \`.prismrules\` file in the root of your workspace.
								`} chatMessageLocation={undefined} />
									...
									<div className='my-4'>
										...
										<div className='text-void-fg-3 text-xs mt-1'>
											{`When disabled, Prism will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
