...
The Prism codebase is not as intimidating as it seems!

Most of Prism's code lives in the folder `src/vs/workbench/contrib/prism/`.

The purpose of this document is to explain how Prism's codebase works. If you want build instructions instead, see [Contributing](https://github.com/prismeditor/prism/blob/main/HOW_TO_CONTRIBUTE.md).
...
Here's a picture of all the dependencies that are relevent between the time you first send a message through Prism's sidebar, and the time a request is sent to your provider.
Sending LLM messages from the main process aprisms CSP issues with local providers and lets us use node_modules more easily.
...
Prism has two types of Apply: **Fast Apply** (uses Search/Replace, see below), and **Slow Apply** (rewrites whole file).
...
>>>>>>> UPDATED
```
This is what allows Prism to quickly apply code even on 1000-line files. It's the same as asking the LLM to press Ctrl+F and enter in a search/replace query.
...
When Prism wants to change your code, it just writes to a text model. This means all you need to know to write to a file is its URI - you don't have to load it, save it, etc. There are some annoying background URI/model things to think about to get this to work, but we handled them all in `prismModelService`.

### Prism Settings Inner Workings
We have a service `prismSettingsService` that stores all your Prism settings (providers, models, global Prism settings, etc). Imagine this as an implicit dependency for any of the core Prism services:
...
### Build process
If you want to know how our build pipeline works, see our build repo [here](https://github.com/prismeditor/prism-builder).



## VSCode Codebase Guide

For additional references, the Prism team put together this list of links to get up and running with VSCode.
<details>
...
Prism is no longer an extension, so these links are no longer required, but they might be useful if we ever build an extension again.
